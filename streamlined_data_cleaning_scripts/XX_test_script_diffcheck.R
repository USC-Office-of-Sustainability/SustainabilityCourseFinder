...
# Read the CSV files
csv1 <- read.csv("streamlined_data/generated_shiny_app_data/usc_courses_full.csv", stringsAsFactors = FALSE)
csv2 <- read.csv("shiny_app/usc_courses_full.csv", stringsAsFactors = FALSE)

# Specify the columns to consider for comparison
columns_to_compare <- c("courseID", "course_title", "section", "school", "session", "instructor",
                        "course_description", "section_name", "department", "origin", "total_enrolled",
                        "N.Sections", "semester", "course_desc", "year", "all_semesters",
                        "course_level", "clean_course_desc", "text", "all_keywords", "all_goals",
