...
ff <- list.files("streamlined_data/SOC_txt_files_pending", 
                 full.names = TRUE)
...
readOneFile <- function(filename) {
  ...
  cleanfile <- paste0("streamlined_data/SOC_txt_files_cleaned/", strsplit(basename(filename), "_")[[1]][1], ".csv")
...
ff <- list.files("streamlined_data/SOC_txt_files_cleaned", 
                 pattern = "csv", full.names = TRUE)
# read data
...
write.csv(combined_data,
          "streamlined_data/01_A1_20251_only.csv",
          row.names = FALSE)
