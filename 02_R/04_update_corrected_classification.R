# after manual fixes
reviewed <- read.csv("USC_AY20-24_Sustainability_Courses_four_categories_1_29_24.csv")
...
new_ge %>%
  select(-Corrected_Sustainability_Classification) -> new_ge
write.csv(new_ge,
...
new_recent %>%
  select(-Corrected_Sustainability_Classification) -> new_recent
...
new_full %>%
  select(-Corrected_Sustainability_Classification) -> new_full
write.csv(new_full,
...
# should enrolled = 0 be shown in the dashboard? no
